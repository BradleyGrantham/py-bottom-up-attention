...
D2_ROOT = os.path.join(PACKAGE_LOCATION, "detectron2/model_zoo/") # Root of detectron2
MIN_BOXES = 36
MAX_BOXES = 36
...
def dump_features_to_tsv(out_dir, dataset_name, detector, pathXid, cuda=True):
    ...
    for i, (img, image_id, instances, features) in enumerate(zip(imgs, img_ids, instances_list, features_list)):
        ...
        d["features"] = base64.b64encode(embeddings_for_oscar.flatten()).decode()
        brads_features_list.append((image_id, json.dumps(d)))

        brads_labels_list.append((image_id, json.dumps([{
            "class": class_names[id_],
            "rect": [float(a) for a in list(bbox)],
            "conf": float(conf.item()),
        } for id_, conf, bbox in zip(clses, preds, boxes)])))
        fake_captions.append({"image_id": image_id, "id": i, "caption": "A photo that has no caption."})
    ...
    tsv_writer(brads_features_list, os.path.join(f"{out_dir}", feature_filename))
    ...
    with open(os.path.join(out_dir, coco_format_caption_filename), "w") as f:
        json.dump({"annotations": fake_captions, "images": fake_images, "type": "captions", "info": "dummy", "licenses": "dummy"}, f)
